...
// NewPeerHandler ...
func NewPeerHandler(logger *zap.Logger, app app.Control, store dao.Store, qosc *qos.Class) vnic.PeerHandler {
	return func(peer *vnic.Peer) {
		...
		p := app.Peer().Add(peer, &client{
			...
			replication: replicationv1.NewReplicationPeerClient(rc),
		})
		...
		replicationv1.RegisterReplicationPeerService(s, &replicationService{p, app})
...
func (c *client) Replication() *replicationv1.ReplicationPeerClient { return c.replication }
