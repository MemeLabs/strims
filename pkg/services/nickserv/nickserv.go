...
func NewNickServ(logger *zap.Logger, svc *vpn.Client, salt []byte, kvStore kv.RWStore, nickservID uint64) (*NickServ, error) {
	...
	b, err := proto.Marshal(&pb.NetworkAddress{
		HostId: svc.Host.VNIC().ID().Bytes(nil),
		Port:   uint32(port),
	})
	if err != nil {
		return nil, err
...
func (s *NickServ) toSignedToken(r *pb.NickservNick) (*pb.NickServToken, error) {
	...
	err = signNickToken(token, s.svc.Host.VNIC().Key())
	return token, err
}
...
func NewNickServClient(svc *vpn.Client, key, salt []byte) (*NickServClient, error) {
	port, err := svc.Network.ReservePort()
	if err != nil {
		return nil, err
	}
...
// func (c *NickServClient) syncAddr(ctx context.Context, svc *vpn.Client, key, salt []byte) {
