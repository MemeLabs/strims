package cmd

import (
	"encoding/json"
	"fmt"
	"io"
	"log"
	"os"
	"os/exec"
	"path"
	"runtime"
	"time"

	"github.com/MemeLabs/go-ppspp/infra/pkg/node"
	"github.com/spf13/cobra"
)

func init() {
	addCmd.PersistentFlags().String("provider", "digitalocean", "hosting provider")
	addCmd.PersistentFlags().String("region", "sfo2", "hosting provider")

	rootCmd.AddCommand(addCmd)
}

var addCmd = &cobra.Command{
	Use:   "add",
	Short: "Add node",
	Long:  `Add node`,
	RunE: func(cmd *cobra.Command, args []string) error {
		// d := node.NewDigitalOceanDriver("")

		// node, err := d.Create(context.Background(), &node.CreateRequest{
		// 	Name:   "test",
		// 	Region: "sfo2",
		// 	Size:   "s-3vcpu-1gb",
		// 	SSHKeys: []string{
		// 		"ssh-rsa  slugalisk@gmail.com",
		// 	},
		// })
		// if err != nil {
		// 	return err
		// }

		// jsonDump(node)

		nodeJSON := `{
			"provider_id": 202465336,
			"name": "test",
			"memory": 1024,
			"vcpus": 3,
			"networks": {
				"v4": [
					"167.71.154.110"
				],
				"v6": [
					"2604:a880:2:d1::112:1"
				]
			},
			"status": "active",
			"region": {
				"Name": "sfo2",
				"LatLng": {
					"Lat": 0.658970003041483,
					"Lng": -2.137033496019417
				}
			}
		}
		`
		var n node.Node
		json.Unmarshal([]byte(nodeJSON), &n)

		sshKeyPath := "/home/slugalisk/.ssh/id_rsa-slugalisk"

		c := exec.Command("ssh", []string{
			"-o", "UserKnownHostsFile=/dev/null",
			"-o", "StrictHostKeyChecking=no",
			"-i", sshKeyPath,
			fmt.Sprintf("root@%s", n.Networks.V4[0]),
			"touch ./test-file",
		}...)

		log.Println(c.String())

		go relayStdio(c)

		if err := c.Run(); err != nil {
			return err
		}

		return nil
	},
}

func jsonDump(i interface{}) {
	_, file, line, _ := runtime.Caller(1)
	b, err := json.MarshalIndent(i, "", "  ")
	if err != nil {
		panic(err)
	}
	fmt.Printf(
		"%s %s:%d: %s\n",
		time.Now().Format("2006/01/02 15:04:05.000000"),
		path.Base(file),
		line, string(b),
	)
}

func relayStdio(cmd *exec.Cmd) error {
	stderr, err := cmd.StderrPipe()
	if err != nil {
		return err
	}
	stdout, err := cmd.StdoutPipe()
	if err != nil {
		return err
	}

	copy := func(w io.Writer, r io.Reader) {
		if _, err := io.Copy(w, r); err != nil {
			panic(err)
		}
	}

	go copy(os.Stderr, stderr)
	go copy(os.Stdout, stdout)
	return nil
}
